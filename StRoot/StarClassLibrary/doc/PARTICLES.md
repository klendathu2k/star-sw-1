...
## PDG Monte Carlo encoding (`StarPDGEncoding.hh`)

The standard PDG encoding for ordinary particles is used throughout.
`StarPDGEncoding.hh` adds STAR-specific constants and a helper function
for heavy ions and hypernuclei that are not covered by the PDG table.

### `hid(z, a, l=0)` — heavy-ion encoding

Returns the PDG encoding for a nucleus with charge *z*, mass number *a*, and
*l* embedded Λ hyperons (default 0 for ordinary nuclei):

```
PDG = 1 000 0L0 ZZZ A0
    = 1000000000 + 10·l·1000000 + z·1000 + a·10
```

Examples:

| Call | Result | Particle |
|------|--------|----------|
| `hid(1,2)` | 1000010020 | deuteron |
| `hid(2,3)` | 1000020030 | ³He |
| `hid(2,4)` | 1000020040 | ⁴He (alpha) |
| `hid(1,3,1)` | 1010001030 | H4Λ (hypertriton-4) |
| `hid(2,4,1)` | 1010002040 | He5Λ |

### Named constants

| Constant | Value | Particle |
|----------|-------|---------|
| `kTriton` | 1 000 000 000 | triton (³H) — legacy constant; prefer `hid(1,3)` |
| `kHyperTriton` | 1 000 000 001 | hypertriton (³ΛH) |
| `kAntiHyperTriton` | 1 000 000 002 | anti-hypertriton |
| `kDalitz` | 1 000 000 111 | Dalitz decay photon |
| `kLambda1520` | 20 003 122 | Λ(1520) resonance |

> **Note on `kTriton`, `kHyperTriton`, `kAntiHyperTriton`:** These three
> constants pre-date the `hid()` function and use an ad-hoc scheme
> (sequential integers from 10⁹) that is inconsistent with the standard
> nuclear PDG formula.  The corresponding `StTriton`, `StHyperTriton`, and
> `StAntiHyperTriton` classes carry these legacy codes.
