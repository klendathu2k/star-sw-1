/// \file StArray.h
/// \brief Dynamic array containers used throughout the STAR event model.
///
/// \details Defines StObjArray (owning or non-owning vector of TObject pointers),
/// StStrArray (owning variant that deletes elements on clear/destruct),
/// StRefArray (non-owning reference array), and the StCollectionDef /
/// StCollectionImp helper macros that generate type-safe StPtrVec<T> and
/// StSPtrVec<T> specialisations for any StObject subclass.
///
/// Also provides StObjLink and the StLink<T> smart-pointer template used to
/// cross-link StEvent objects within a single branch.
#ifndef _StRegistry_
#define _StRegistry_
...
/**
 * \class StObjLink
 * \brief Non-owning pointer wrapper for a single StObject, storable in ROOT.
 *
 * Provides pointer semantics (operator*, operator->) and ROOT persistence via
 * ClassDef.  Use StLink<T> for a type-safe variant.
 */
...
/**
 * \class StObjArray
 * \brief ROOT-persistent dynamic array of TObject pointers.
 *
 * \details Wraps a \c std::vector<TObject*> (fV) with an interface that mirrors
 * a subset of \c std::vector while also providing ROOT I/O via ClassDef.
 * Objects are \e not owned by default; use StStrArray if ownership is needed.
 *
 * The StCollectionDef() macro generates a type-safe StPtrVec<T> subclass
 * (non-owning) and an StSPtrVec<T> subclass (owning) for any StObject-derived
 * type T.
 */
class StObjArray : public StObject {
public:
             /// \param sz  Reserve space for \a sz pointers on construction.
...
virtual void Browse(TBrowser *b);
        ...
        /// Remove all elements (does not delete pointed-to objects).
        void   clear()          {fV.clear()          ;}
...
virtual Bool_t IsFolder() const;
        /// \return Reference to the pointer at index \a i (no bounds check).
        TObject *       &at(int i)       {return fV[i];}
        /// Replace the element at index \a i with \a obj.
...
/**
 * \class StStrArray
 * \brief Owning variant of StObjArray — deletes pointed-to objects on clear/destruct.
 *
 * \details Used when the array is the sole owner of its elements (e.g. the
 * StSPtrVec<T> collections generated by StCollectionDef()).
 */
...
/**
 * \class StRefArray
 * \brief Non-owning reference array for cross-branch pointer collections.
 *
 * \details Elements are written to the ROOT file as unique IDs during the
 * write pass and resolved back to live pointers by StXRefManager during the
 * read pass.  The array does not delete its elements.
 */
